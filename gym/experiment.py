...
def experiment(
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        for i in range(batch_size):
            ...
            if si > 0:
                a.append(traj['actions'][si-1:si-1 + s[-1].shape[1]].reshape(1, -1, act_dim))
#                 print (traj['actions'].shape)
            else:
                a.append(np.concatenate([np.ones((1, act_dim)) * -10., traj['actions'][si:si-1 + max_len]], axis=0).reshape(1, -1, act_dim))
#                 print (a[-1].shape)
...
#         d = torch.from_numpy(np.concatenate(d, axis=0)).to(dtype=torch.long, device=device)
    ...
    if 'dt' in model_type or 'star' in model_type:
        trainer = SequenceTrainer(
            ...
            eval_fns=[eval_episodes(tar, writer) for tar in env_targets],
        )
    elif model_type == 'bc':
        trainer = ActTrainer(
            model=model,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default='star') # starformer
    ...
    experiment('gym-experiment', variant=vars(args), args=args)
